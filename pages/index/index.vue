<template>
	<view class="content">
		...
		<view style="margin: 20rpx 2%;" v-if="!showEmpty"><u-loadmore :status="status" /></view>
	...
	methods: {
		async loadData(type) {
			...
			await getData(this.listQuery).then(res => {
				...
				if (paginator) {
					this.listQuery.page = paginator.next
			...
			//判断是否还有下一页
			this.status = endPage - currentPage === 0 || this.list.length === 0 ? 'nomore' : 'loadmore'
